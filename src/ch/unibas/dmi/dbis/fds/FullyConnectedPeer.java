...
/**
 * Example peer implementation class implements a fully connected peer.
 * GetQueries are done via broadcasting to each node.
 * SaveQueries are done at each node.
 *
...
public class FullyConnectedPeer extends PeerNode {
    ...
    /**
     * In this fully connected network a data item may be saved at any peer.
     * So no save queries are needed to be passed to other peers.
     *
    ...
    /**
     * In the fully connected network a broadcast to all peers is sent to get a data item if not already available and
     * query is coming from the client.
     *
     * @param originOfQuery the peer that issued the query (null if client app)
     * @param key           of data item
    ...
    /**
     * @see ch.unibas.dmi.dbis.fds.PeerNode#lookupNodeForItem(PeerNode, String)
     */
    @Override
