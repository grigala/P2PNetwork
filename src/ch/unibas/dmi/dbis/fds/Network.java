...
public abstract class Network {
    ...
    private class GuiPanel extends javax.swing.JPanel {
        ...
        /**
         * Helping method for paintNetwork to retrieve the angle of a peer at
         * the illustration circle
         *
    ...
    /**
     * Used to log passed messages for statistics. Messages are NOT logged if from and to are identical (i.e., local
     * call).
     *
    ...
    /**
     * This method allows to check if a given hashID is within a sector of a chord identifier ring. Attention "within"
     * means that if hashID is equal to start or end it is also within.
     *
     ...
     * @param startInclusive set to <tt>true</tt> if the start value should be included (closed interval: "["); set to
     *                       <tt>false</tt> if the start value should be excluded (open interval: "(").
     * @param endInclusive   set to <tt>true</tt> if the end value should be included (closed interval: "]"); set to
     *                       <tt>false</tt> if the end value should be excluded (open interval: ")").
    ...
    public boolean isHashElementOf(long hash, long start, long end, boolean startInclusive, boolean endInclusive) {
        ...
        if (!startInclusive && !endInclusive) {
            ...
            if (start == (1 << numberOfBits) - 1 && end == 0) return false;
        }
        if (!startInclusive) {
            start++;
            if (start == (1 << numberOfBits)) start = 0;
        }
        if (!endInclusive) {
            end--;
            if (end == -1) end += (1 << numberOfBits);
        }
